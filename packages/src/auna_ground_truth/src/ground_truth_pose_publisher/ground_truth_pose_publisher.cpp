...
void GroundTruthPosePublisher::timer_callback()
{
  ...
  try {
    ...
    RCLCPP_DEBUG(this->get_logger(), "Successfully found gazebo_world->base_link transform");
  ...
  localization_pose.header.frame_id = "gazebo_world";
  ...
  RCLCPP_DEBUG(
    this->get_logger(),
    "Publishing gazebo_pose: position [%.2f, %.2f, %.2f], orientation [%.2f, %.2f, %.2f, %.2f]",
