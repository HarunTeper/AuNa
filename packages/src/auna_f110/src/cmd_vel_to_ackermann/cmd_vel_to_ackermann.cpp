...
CmdVelToAckermann::CmdVelToAckermann() : Node("cmd_vel_to_ackermann_node")
{
  ...
  ackermann_publisher =
    this->create_publisher<ackermann_msgs::msg::AckermannDriveStamped>("ackermann_cmd", 2);
...
void CmdVelToAckermann::cmd_vel_callback(const geometry_msgs::msg::Twist::SharedPtr msg)
{
  ...
  if (convert_yaw_to_steering_angle_) {
    if (fabs(msg->linear.x) < 0.001) {
      ...
      // use ackermann_msg.drive.steering_angle = atan(wheelbase_ * msg->angular.z / msg->linear.x);
      // to calculate the steering angle for positive velocies, but swap sign for negative
      // velocities
