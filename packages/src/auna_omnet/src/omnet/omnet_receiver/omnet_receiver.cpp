...
{
  omnet_subscriber_ = this->create_subscription<auna_its_msgs::msg::CAM>(
    "cam_in", 2, [this](const auna_its_msgs::msg::CAM::SharedPtr msg) { cam_callback(msg); });
...
void OmnetReceiver::cam_callback(const auna_its_msgs::msg::CAM::SharedPtr msg)
{
  ...
  cacc_msg.theta =
    (((float)msg->high_frequency_container.heading.value / 10) * M_PI / 180 -
     (2 * M_PI) * floor(
                    ...
                    (M_PI)));  // 0.1 degree
  cacc_msg.thetadot =
    ((float)msg->high_frequency_container.yaw_rate.value / 100) * M_PI / 180;  // 0.01degree/s
  cacc_msg.v = (float)msg->high_frequency_container.speed.value / 100 / scale_factor_;  // 0.01 m/s
  cacc_msg.vdot = (float)msg->high_frequency_container.longitudinal_acceleration.value / 10 /
