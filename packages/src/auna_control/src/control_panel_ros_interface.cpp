...
void ControlPanelROSInterface::initializeROS()
{
  ...
  RCLCPP_DEBUG(node_->get_logger(), "Initializing ROS node and creating clients/subscribers");
...
void ControlPanelROSInterface::createClientsAndSubscribers()
{
  ...
  if (node_) {
    RCLCPP_DEBUG(
      node_->get_logger(), "Creating clients and subscribers for namespace: '%s'",
      ns_prefix.c_str());
  }
  ...
  odom_subscriber_ = node_->create_subscription<nav_msgs::msg::Odometry>(
    ns_prefix + "/odom", 10,
    std::bind(&ControlPanelROSInterface::onOdometryReceived, this, std::placeholders::_1));
  cmd_vel_subscriber_ = node_->create_subscription<geometry_msgs::msg::Twist>(
    ns_prefix + "/cmd_vel_twist", 10,
    std::bind(&ControlPanelROSInterface::onCmdVelReceived, this, std::placeholders::_1));
  imu_subscriber_ = node_->create_subscription<sensor_msgs::msg::Imu>(
    ns_prefix + "/imu", 10,
    std::bind(&ControlPanelROSInterface::onImuReceived, this, std::placeholders::_1));
}
...
void ControlPanelROSInterface::triggerEstop(bool activate)
{
  if (!estop_set_client_ || !estop_set_client_->service_is_ready()) {
    if (node_) {
      RCLCPP_DEBUG(
        node_->get_logger(), "E-Stop service not available when trying to %s E-Stop",
...
void ControlPanelROSInterface::setCmdVelSource(const QString & source)
{
  ...
  set_source_client_->async_send_request(
    request,
    std::bind(&ControlPanelROSInterface::onSetSourceResponse, this, std::placeholders::_1));
}
...
void ControlPanelROSInterface::checkServices()
{
  ...
  if (node_) {
    RCLCPP_DEBUG(
      node_->get_logger(), "Checking services. Backend ready: %s", ready ? "true" : "false");
  }
  ...
  if (ready) {
    ...
    estop_status_client_->async_send_request(
      request,
      std::bind(&ControlPanelROSInterface::onEstopStatusResponse, this, std::placeholders::_1));
  }
}
...
void ControlPanelROSInterface::checkInputSources()
{
  if (input_sources_client_ && input_sources_client_->service_is_ready()) {
    ...
    input_sources_client_->async_send_request(
      request,
      std::bind(&ControlPanelROSInterface::onInputSourcesResponse, this, std::placeholders::_1));
  }
}
...
void ControlPanelROSInterface::checkSourceStatus()
{
  if (source_status_client_ && source_status_client_->service_is_ready()) {
    ...
    source_status_client_->async_send_request(
      request,
      std::bind(&ControlPanelROSInterface::onSourceStatusResponse, this, std::placeholders::_1));
  }
}
...
void ControlPanelROSInterface::onEstopStatusResponse(rclcpp::Client<Trigger>::SharedFuture future)
{
  auto response = future.get();
  bool is_active = response->message == "Emergency stop is ACTIVE";
  if (node_) {
    RCLCPP_DEBUG(node_->get_logger(), "Received E-Stop status: %s", response->message.c_str());
  }
  emit estopStatusUpdated(is_active, QString::fromStdString(response->message));
}

void ControlPanelROSInterface::onInputSourcesResponse(rclcpp::Client<Trigger>::SharedFuture future)
{
  auto response = future.get();
  if (response->success) {
    if (node_) {
      RCLCPP_DEBUG(node_->get_logger(), "Received input sources: %s", response->message.c_str());
    }
    QStringList sources = QString::fromStdString(response->message).split(",", Qt::SkipEmptyParts);
    emit inputSourcesUpdated(sources);
  }
}

void ControlPanelROSInterface::onSourceStatusResponse(rclcpp::Client<Trigger>::SharedFuture future)
{
  auto response = future.get();
  if (response->success) {
    if (node_) {
      RCLCPP_DEBUG(node_->get_logger(), "Received source status: %s", response->message.c_str());
    }
    QString current = QString::fromStdString(response->message).section(": ", 1);
...
void ControlPanelROSInterface::onOdometryReceived(const nav_msgs::msg::Odometry::SharedPtr msg)
{
...
void ControlPanelROSInterface::onCmdVelReceived(const geometry_msgs::msg::Twist::SharedPtr msg)
{
  if (node_) {
    RCLCPP_DEBUG(
      node_->get_logger(), "Received cmd_vel: linear=%.2f, angular=%.2f", msg->linear.x,
      msg->angular.z * 180.0 / M_PI);
  }
  emit cmdVelUpdated(msg->linear.x, msg->angular.z * 180.0 / M_PI);
}

void ControlPanelROSInterface::onImuReceived(const sensor_msgs::msg::Imu::SharedPtr msg)
{
  if (node_) {
    RCLCPP_DEBUG(
      node_->get_logger(), "Received IMU: ax=%.2f, ay=%.2f, az=%.2f", msg->linear_acceleration.x,
