...
# Pre-download Gazebo models for faster startup
RUN mkdir -p /home/ubuntu/.gazebo/models
RUN cd /home/ubuntu/.gazebo/models && \
    echo "Downloading Gazebo models from GitHub repository..." && \
    wget -q --timeout=30 --tries=2 -O gazebo_models.tar.gz \
    https://github.com/osrf/gazebo_models/archive/master.tar.gz && \
    tar -xzf gazebo_models.tar.gz --strip-components=1 && \
    rm gazebo_models.tar.gz && \
    echo "Gazebo models downloaded successfully" || \
    echo "Failed to download models, will use online database at runtime"

# Source Gazebo setup in bashrc
